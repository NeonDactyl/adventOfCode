	...
	def run_program(self, input_signal):
		...
		while running:
			...
			if current_code['code'] in [1, 2, 7, 8]:
				if current_code['first'] == 1:
					val1 = self.numbers[self.current_index + 1] 
				elif current_code['first'] == 2:
					val1 = self.numbers[self.numbers[self.current_index + 1] + self.relative_base]
				else:
					val1 = self.numbers[self.numbers[self.current_index + 1]]
				if current_code['second'] == 1:
					val2 = self.numbers[self.current_index + 2]
				elif current_code['second'] == 2:
					val2 = self.numbers[self.numbers[self.current_index + 2] + self.relative_base]
				else:
					val2 = self.numbers[self.numbers[self.current_index + 2]]
				if current_code['third'] == 1:
					result_index = self.numbers[self.numbers[self.current_index + 3]] 
				elif current_code['third'] == 2:
					result_index = self.numbers[self.numbers[self.current_index + 3] + self.relative_base]
				else:
					result_index =  self.numbers[self.current_index + 3]
			...
			elif current_code['code'] == 9:
				if self.current_code['third'] == 1
				self.relative_base += self.numbers[
			else:
